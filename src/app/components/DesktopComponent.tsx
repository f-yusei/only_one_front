...
function extractDR(
  data: { [key: string]: { [key: string]: { [key: string]: boolean[] } | boolean[] } },
  char: string
) {
      ...
      for (const dormitory of dormitories) {
        try {
          const response = await fetch(
            `${NEXT_PUBLIC_API_URL}/api/dashboard?dormitory=${encodeURIComponent(dormitory)}`
          );
          if (!response.ok) {
    ...
    <Flex height="90vh" width={'100%'}>
      <Box style={gridContainerStyle}>
        ...
        <div style={gridItemStyle}>
          {bathArray ? <DisplayPublicBath numberOfUsingBathData={bathArray} /> : <p>Loading...</p>}
        </div>
        <div style={gridItemStyle}>
          {showerArray ? <DisplayShower showerData={showerArray} /> : <p>Loading...</p>}
        </div>
        <div style={gridItemStyle}>
          {washArray ? <DisplayWasher washerData={washArray} /> : <p>Loading...</p>}
        </div>
        <div style={gridItemStyle}>
          {dryArray ? <DisplayDryer dryerData={dryArray} /> : <p>Loading...</p>}
        </div>
      </Box>
    </Flex>
  );
